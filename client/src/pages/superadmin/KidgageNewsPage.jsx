// client/src/pages/superadmin/KidgageNewsPage.jsx
...
function EmptyState({ onAdd }) {
  return (
    <div className="rounded-[28px] border border-dashed border-slate-200 bg-slate-50 px-4 py-16 text-center">
      ...
      <h3 className="mt-5 text-lg font-black text-slate-900">No news found</h3>
      <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
        Create your first KidGage news update to publish platform announcements,
        stories, or activity updates.
      </p>
...
function NewsModal({
  ...
  const title = mode === "edit" ? "Edit News" : "Add News";
      ...
      <div className="mx-auto my-4 w-full max-w-5xl overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-2xl sm:my-8 sm:rounded-[32px]">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 px-4 py-5 sm:px-6">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm leading-6 text-slate-500">
              Manage news title, image, publish date, article content, and
              public visibility.
            </p>
          </div>
        ...
        <div className="grid gap-6 px-4 py-5 sm:px-6 lg:grid-cols-[1fr_420px]">
          <div className="space-y-5">
            <Field label="News Title" hint="Required">
              <Input
                value={form.title}
              ...
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <div className="mt-1 text-sm text-slate-500">
                    Active news can appear on the public website.
                  </div>
                </div>
...
function NewsDetailsModal({ open, item, onClose }) {
  const [imgError, setImgError] = useState(false);
...
function NewsCard({ item, onToggle, onEdit, onDelete, onView, toggling }) {
  const [imgError, setImgError] = useState(false);
        ...
        <div className="min-w-0 p-5 sm:p-6">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="min-w-0">
              ...
              <div className="mt-3 inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-bold text-slate-600 ring-1 ring-slate-200">
...
export default function KidgageNewsPage() {
  ...
  const loadNews = useCallback(async ({ silent = false } = {}) => {
    try {
      ...
      const { data } = await api.get("/super-admin/news");
  ...
  const stats = useMemo(() => {
    ...
    const active = news.filter((item) => item.active).length;
    const inactive = news.filter((item) => !item.active).length;
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (modalMode === "edit") {
        await api.put(`/super-admin/news/${form.id}`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        setMessage("News updated successfully.");
      } else {
        await api.post("/super-admin/news", payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
  ...
  async function handleToggle(id) {
    const current = news.find((x) => x.id === id);
    if (!current) return;
    ...
    try {
      ...
      await api.patch(`/super-admin/news/${id}/toggle`, {
        active: nextActive,
      });
    } catch (err) {
      setError(err?.response?.data?.message || "Failed to update news status.");
      await loadNews({ silent: true });
    } finally {
      setTogglingId("");
    }
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      await api.delete(`/super-admin/news/${id}`);
    ...
    <main className="min-h-screen w-full bg-slate-50 px-3 py-4 text-slate-900 sm:px-5 md:px-6 lg:px-8">
            ...
            <div className="relative flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-2 max-w-3xl text-sm leading-7 text-slate-500">
                  Create and publish platform news, announcements, activity
                  updates, stories, and public communication posts.
                </p>
              </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Search news title, description, date..."
          ...
          <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">
                Showing {filteredNews.length} of {news.length} posts.
              </p>
            </div>
      ...
      <ConfirmDialog
        ...
        message={`This will permanently delete ${
          deleteState?.item?.title || "this news post"
